...
Each generated poster keeps the original angled status ribbon and adds a restrained information strip beneath it. Watching cards show **NEW · S2 E11** and **NEXT · S2 E5** side by side, so the newest release and your next unwatched episode remain distinct without covering a large part of the artwork. Plan to Watch and revived Completed cards use one compact strip. Every badge includes both season and episode, with a strong black outline and shadow for readability across phone, desktop, and Android TV.
...
## Updating from 1.8.2 to 1.8.5

Version 1.8.5 keeps the proven 1.8.2 metadata, grouping, English-title, and watched-history behavior unchanged. Only poster labelling is revised: the original angled status ribbon remains, the two Watching details are placed side by side in one compact row, and every label now includes both season and episode (for example **S2 E11**). There is one universal manifest and one universal poster design because Nuvio addon installations sync across a profile rather than per device. The poster style version changed, so the next refresh publishes new poster URLs automatically.
